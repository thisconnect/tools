...
test('build CSS to build/dirs', t => {
  ...
  buildCSS({ src, dest, assets: '../assets', fonts: '../fonts' })
    ...
    .then(styles =>
      t.ok(
        styles.indexOf(`url('../fonts/fontawesome-`) > -1,
        "path begins with url('../fonts/"
      )
    )
    .then(() =>
      readFile(dest.replace('.css', '.min.css'), { encoding: 'utf8' })
    )
    .then(min =>
      t.ok(
        min.indexOf(`url(../fonts/fontawesome-`) > -1,
        'minified path begins with url(../fonts/'
      )
    )
    .then(() => t.end())
    .catch(t.end);
